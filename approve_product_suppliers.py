#!/usr/bin/env python3
"""Approve and execute all pending product supplier proposals."""
...
def approve_and_execute(token):
    ...
    # Get all pending proposals for PRODUCT_SUPPLIER
    resp = requests.get('http://localhost:3050/api/master-data/entity/PRODUCT_SUPPLIER?status=PENDING', headers=headers)
    ...
    print(f"Found {len(proposals)} pending PRODUCT_SUPPLIER proposals")
    ...
    for p in proposals:
        ...
        # Execute the proposal (create the product supplier)
...
def main():
    ...
    print("\nApproving and executing product supplier proposals...")
